...
export default function ProjectSettingsModal({ project, onClose, onSaved, onDeleted, onArchived }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (file) {
      ...
      const { error: uploadError } = await supabase.storage.from('project-logos').upload(path, file)
      if (uploadError) { setError(uploadError.message); setLoading(false); return }
      const { data: urlData } = supabase.storage.from('project-logos').getPublicUrl(path)
      logo_url = urlData.publicUrl
    }
    const { error: updateError } = await supabase.from('projects').update({ color, logo_url }).eq('id', project.id)
    setLoading(false)
