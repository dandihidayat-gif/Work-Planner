...
// Strip non-Latin1 chars so jsPDF WinAnsi encoding doesn't produce &x& garbage
function safe(str) {
  if (!str) return ''
  return String(str).replace(/[^\x00-\xFF]/g, '').replace(/&/g, 'and')
}
...
export default function ExportWorkReportModal({ projects, onClose }) {
  ...
  const handleExport = async () => {
    ...
    const { data: posts } = await supabase
      .from('post_schedules').select('*')
      .in('project_id', selectedProjects)
      .gte('post_date', firstDay).lte('post_date', lastDay)
      .order('post_date')
    ...
    doc.text(safe(periodLabel), W - marginR - doc.getTextWidth(safe(periodLabel)), 30)
    y = 66
    ...
    WEEKDAYS_SHORT.forEach((wd, i) => {
      ...
      doc.setFontSize(8)
      doc.setFont(undefined, 'bold')
      doc.text(safe(wd), marginL + i * calCellW + calCellW / 2, calTop + 11, { align: 'center' })
    })
    y = calTop + 16
    ...
    for (let row = 0; row < days.length / 7; row++) {
      for (let col = 0; col < 7; col++) {
        ...
        if (isToday(d)) doc.setFillColor(...LIGHT_BLUE)
        ...
        if (d) {
          ...
          dayPosts.slice(0, 2).forEach((post, pi) => {
            ...
            doc.setFontSize(6.5)
            doc.setFont(undefined, 'bold')
            doc.text(safe(proj?.name || ''), cx + 5, chipY + 7.5, { maxWidth: calCellW - 10 })
          })
    ...
    if ((posts || []).length === 0) {
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          0: { cellWidth: 100 }, 1: { cellWidth: 180 }, 2: { cellWidth: 70 },
          3: { cellWidth: 80 }, 4: { cellWidth: 120 }, 5: { cellWidth: 80 },
        },
        margin: { left: marginL, right: marginR },
        didParseCell: (data) => {
          if (data.section === 'body' && data.column.index === 5 && data.cell.raw === 'Posted') {
    ...
    for (const proj of selectedProjectsData) {
      ...
      if (taskY > H - 120) { doc.addPage(); taskY = 36 }
      ...
      if (logo) {
        try { doc.addImage(logo, 'PNG', marginL + 6, taskY + 4, 20, 20) } catch {}
      }
      ...
      doc.setFontSize(11)
      doc.setFont(undefined, 'bold')
      doc.text(safe(proj.name), marginL + logoOffset, taskY + 18)
      ...
      if (projTasks.length === 0) {
        ...
        const taskRows = projTasks.map(t => [
          safe(t.title),
          safe(t.no_deadline ? 'No Deadline' : (t.deadline || '-')),
          safe(t.completed_at || '-'),
          t.status === 'done' ? 'Done' : t.status === 'on_progress' ? 'On Progress' : 'Pending',
          safe(t.final_link || '-'),
          safe(t.completion_note || '-'),
        ])
        ...
        autoTable(doc, {
          ...
          columnStyles: {
            0: { cellWidth: 150 }, 1: { cellWidth: 75 }, 2: { cellWidth: 90 },
            3: { cellWidth: 75 }, 4: { cellWidth: 180 }, 5: { cellWidth: 'auto' },
          },
          margin: { left: marginL, right: marginR },
          didParseCell: (data) => {
      ...
      <div className="modal-overlay" onClick={onClose}>
        <div className="modal" style={{ maxWidth: 500 }} onClick={(e) => e.stopPropagation()}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 20 }}>
            PDF berisi: kalender + jadwal posting (halaman 1), lalu rekap task per project.
          </p>
          ...
          <div className="field-group">
            ...
            <div style={{ marginBottom: 8 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                <input type="checkbox" checked={selectedProjects.length === projects.length} onChange={toggleAll} />
                Semua Project
              </label>
            </div>
